...
split = [0]
def fill_split(max_N):
    for k in range(1, max_N // 2 + 1):
        num = k - 1
        denom1 = (fact[k] * fact[k]) % mod
        denom2 = (2 * (k + 1)) % mod
        denom = (denom1 * denom2) % mod
        split.append((num * inv(denom)) % mod)
        
# ax + by = gcd(a, b) for some x, y
def gcd(a, b):
    if b == 0:
        return a, 1, 0
    g, x, y = gcd(b, a % b)
...
def calc(N):
    ...
    for k in range(2, N//2 + 1):
        ...
        delta = (split[k] * inv(fact[N - 2 * k])) % mod
        ans += delta
    ans = (ans * fact[N]) % mod
    print(ans)
